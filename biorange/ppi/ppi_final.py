...
def plot_ppi_network(nodes, output_dir):
    ...
    pos = nx.kamada_kawai_layout(
        G
    )  #'spring': 弹簧布局（默认）'kamada_kawai'、'circular'、'random'、'shell'、'spectral'、'fruchterman_reingold'
    nx.draw(
        G,
        pos,
        with_labels=True,
    ...
    plt.savefig(os.path.join(output_dir, "ppi_network.pdf"))
    plt.show()
    print(
        f"PPI network graph has been saved to {os.path.join(output_dir, 'ppi_network.png' and 'ppi_network.pdf')}"
    )
...
if __name__ == "__main__":
    ...
    output_dir = "./results/output2/ppi2"
    os.makedirs(output_dir, exist_ok=True)
